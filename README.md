...
時系列の空中写真（同じ範囲・同じ縮尺で切り出したもの）から **森林緩衝帯**（森林と集落の間にある田畑・草地・耕作放棄地などの開放地）を抽出し、
...
2. **森林 / 開放地 / 人工物**: 画素ごとの特徴量（平滑化した輝度・彩度・緑らしさ・局所テクスチャ・局所最小輝度）を、**教師サンプル**（写真上でクリックして置く円）から学習したクラスのプロトタイプと比べ、最も近いクラスに割り当てます（共通分散の最近傍プロトタイプ判定）。サンプルは全時期共通のものと、時期ごとのものを併用できます。森林・開放地のサンプルが無い時期は輝度のしきい値（大津法）で代替します。
3. **後処理**: 森林マスクのクロージング／オープニング（整形半径）と、小さな開放地の除去（最小領域）。
4. **生活空間（住宅地・集落）** は緩衝帯に含めません。画面上で描いた多角形（既定で港周辺の集落など 5 か所を登録）と、任意で「人工物の周囲 N m」を生活空間として除外し、白で表示します。
5. **緩衝帯** = 陸域の開放地から生活空間を除いたもの。必要に応じて「森林境界からの幅 (m)」で森林際の帯だけに限定、解析範囲（多角形）で対象地域を限定、ブラシで手動修正できます。
...
- **森林緩衝帯の範囲**: 陸域の開放地すべて（森林境界からの幅による限定はしない。「森林境界からの幅」は 0 = 全域）。ただし **住宅地・集落は生活空間であり緩衝帯ではない** ため除外する。除外範囲は多角形で指定（既定 5 か所）。
