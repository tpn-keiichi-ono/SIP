...
function drawPhotos(ctx) {
  ...
  state.photos.forEach((p, k) => {
    ...
    if (p.dir != null) { // 撮影方向
      ...
      ctx.strokeStyle = sel ? '#ffd400' : 'rgba(255,255,255,0.9)'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(q.x, q.y); ctx.lineTo(q.x + Math.cos(a) * (r + 7), q.y + Math.sin(a) * (r + 7)); ctx.stroke();
    }
