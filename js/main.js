...
function tick(ts) {
  ...
  const tl = state.timeline;
  if (y >= tl.xMax) {
    if (state.recording) { y = tl.xMax; state.year = y; sceneDirty = true; render(); stopRecording(); return; }
    if ($('loop').checked) y = tl.xMin; else { y = tl.xMax; state.year = y; sceneDirty = true; render(); togglePlay(false); return; }
  }
  state.year = y; sceneDirty = true; render();
...
function startRecording() {
  ...
  $('loop').checked = false; state.year = state.timeline.xMin; sceneDirty = true; render();
  rec.start(200);
  togglePlay(true);
}
...
window.SIP = {
  state,
  setYear: (y) => { setYear(y); sceneDirty = true; render(); },
